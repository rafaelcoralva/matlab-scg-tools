...
% INPUTS:
% avg_s1_xyz (Matrix: [length(avg_s1), 3]) - 3D average S1 acceleration trajectory matrix of size,
%                                            where the three columns correspond to the 3 scg xyz components

% OUTPUTS:
% axes_angles = [el_mech_axis, az_mech_axis, el_antimech_axis, az_antimech_axis], where:
...
idx_triangles_xzy = boundary(avg_s1_xyz(:,1), avg_s1_xyz(:,3), avg_s1_xyz(:,2), 0); % A matrix of triangles that comprises the 3D hull surface. 
...
%% 1.2 Tri-force Interpolation of the 3D hull surface (this part isn't really necessary because TriangleRayIntersection function used later doesn't care about the size of the triangles - however, it usful to facilitate visualization as the triangles' colors indicate radial distance).
...
i = 1;
while (i <= size(idx_triangles_xzy_interp,1)) 
       
    large_tri_vert_1_xyz(i,:) = avg_s1_xyz_interp(idx_triangles_xzy_interp(i,1),:); % The x,y,and z coordinates (in the SCG reference frame) of vertex 1 of the triangles comprising the 3D hull surface.
    large_tri_vert_2_xyz(i,:) = avg_s1_xyz_interp(idx_triangles_xzy_interp(i,2),:); % The x,y,and z coordinates (in the SCG reference frame) of vertex 2 of the triangles comprising the 3D hull surface.
    large_tri_vert_3_xyz(i,:) = avg_s1_xyz_interp(idx_triangles_xzy_interp(i,3),:); % The x,y,and z coordinates (in the SCG reference frame) of vertex 3 of the triangles comprising the 3D hull surface.
    ...
    large_tri_max_length(i) = max([norm(large_tri_vert_2_xyz(i,:) - large_tri_vert_1_xyz(i,:)), norm(large_tri_vert_3_xyz(i,:) - large_tri_vert_2_xyz(i,:)), norm(large_tri_vert_3_xyz(i,:) - large_tri_vert_1_xyz(i,:))]); % Check if large triangle requires interpolation (i.e., if it's actually "large).
    ...
    if (large_tri_area(i) > area_thr) || (large_tri_max_length(i) > length_thr) % Tri-force interpolation - creating 4 smaller triangles from the original triangle.
        ...
        % Small Triangle 1.
        [~,idx_small_tri_1_vert_1] = ismember([small_tri_1_vert_1_xyz(end,1) small_tri_1_vert_1_xyz(end,2) small_tri_1_vert_1_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 1's vertex 1.
        [~,idx_small_tri_1_vert_2] = ismember([small_tri_1_vert_2_xyz(end,1) small_tri_1_vert_2_xyz(end,2) small_tri_1_vert_2_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 1's vertex 2.
        [~,idx_small_tri_1_vert_3] = ismember([small_tri_1_vert_3_xyz(end,1) small_tri_1_vert_3_xyz(end,2) small_tri_1_vert_3_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 1's vertex 3.
        
        idx_small_tri_1_allvertices = [idx_small_tri_1_vert_1, idx_small_tri_1_vert_2, idx_small_tri_1_vert_3];
        
        % Small Triangle 2.
        [~,idx_small_tri_2_vert_1] = ismember([small_tri_2_vert_1_xyz(end,1) small_tri_2_vert_1_xyz(end,2) small_tri_2_vert_1_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 2's vertex 1.
        [~,idx_small_tri_2_vert_2] = ismember([small_tri_2_vert_2_xyz(end,1) small_tri_2_vert_2_xyz(end,2) small_tri_2_vert_2_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 2's vertex 2.
        [~,idx_small_tri_2_vert_3] = ismember([small_tri_2_vert_3_xyz(end,1) small_tri_2_vert_3_xyz(end,2) small_tri_2_vert_3_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 2's vertex 3.
       
        idx_small_tri_2_allvertices = [idx_small_tri_2_vert_1, idx_small_tri_2_vert_2, idx_small_tri_2_vert_3];
        
        % Small Triangle 3.
        [~,idx_small_tri_3_vert_1] = ismember([small_tri_3_vert_1_xyz(end,1) small_tri_3_vert_1_xyz(end,2) small_tri_3_vert_1_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 3's vertex 1.
        [~,idx_small_tri_3_vert_2] = ismember([small_tri_3_vert_2_xyz(end,1) small_tri_3_vert_2_xyz(end,2) small_tri_3_vert_2_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 3's vertex 2.
        [~,idx_small_tri_3_vert_3] = ismember([small_tri_3_vert_3_xyz(end,1) small_tri_3_vert_3_xyz(end,2) small_tri_3_vert_3_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 3's vertex 3.
       
        idx_small_tri_3_allvertices = [idx_small_tri_3_vert_1, idx_small_tri_3_vert_2, idx_small_tri_3_vert_3];
        
        % Small Triangle 4.
        [~,idx_small_tri_4_vert_1] = ismember([small_tri_4_vert_1_xyz(end,1) small_tri_4_vert_1_xyz(end,2) small_tri_4_vert_1_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 4's vertex 1.
        [~,idx_small_tri_4_vert_2] = ismember([small_tri_4_vert_2_xyz(end,1) small_tri_4_vert_2_xyz(end,2) small_tri_4_vert_2_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 4's vertex 2.
        [~,idx_small_tri_4_vert_3] = ismember([small_tri_4_vert_3_xyz(end,1) small_tri_4_vert_3_xyz(end,2) small_tri_4_vert_3_xyz(end,3)],avg_s1_xyz_interp,'rows'); % Index of small triangle 4's vertex 3.
       
        idx_small_tri_4_allvertices = [idx_small_tri_4_vert_1, idx_small_tri_4_vert_2, idx_small_tri_4_vert_3];        
        
        idx_triangles_xzy_interp = insertrows(idx_triangles_xzy_interp,[idx_small_tri_1_allvertices;idx_small_tri_2_allvertices;idx_small_tri_3_allvertices;idx_small_tri_4_allvertices] ,i-1);
        ...
        clear large_tri_midpoint_1_2_xyz large_tri_midpoint_2_3_xyz large_tri_midpoint_3_1_xyz idx_small_tri_1_vert_1 idx_small_tri_1_vert_2 idx_small_tri_1_vert_3 idx_small_tri_1_allvertices idx_small_tri_2_vert_1 idx_small_tri_2_vert_2 idx_small_tri_2_vert_3 idx_small_tri_2_allvertices idx_small_tri_3_vert_1 idx_small_tri_3_vert_2 idx_small_tri_3_vert_3 idx_small_tri_3_allvertices idx_small_tri_4_vert_1 idx_small_tri_4_vert_2 idx_small_tri_4_vert_3 idx_small_tri_4_allvertices 
    end
...
if flag_plot > 0
    ...
    trisurf(idx_triangles_xzy_interp, avg_s1_xyz_interp(:,1), avg_s1_xyz_interp(:,3), avg_s1_xyz_interp(:,2), sqrt(avg_s1_xyz_interp(:,1).^2 + avg_s1_xyz_interp(:,3).^2 + avg_s1_xyz_interp(:,2).^2) ,'FaceAlpha',0.5,'EdgeAlpha',0); xlabel('x (mg)'); ylabel('z (mg)'); zlabel('y (mg)'); hold on;
...
[idx_el_max_diameter_sph, idx_az_max_diameter_sph] = find(diameter_sph_mat == max_diameter); % The row (elevation) and column (azimuth) indices of 'diameter_sph_mat' corresponding to the max diameter (in the anterior hemisphere). 
el_max_diameter_sph_mat = idx_el_max_diameter_sph - 91; % The actual elevation angle (in the Matlab reference frame) maximising diameter. 
az_max_diameter_sph_mat = idx_az_max_diameter_sph - 180; % The actual azimuthal angle (in the Matlab reference frame) maximising diameter.
...
el_min_diameter_sph_mat = idx_min_diameter_el_sph - 91; % The actual elevation angle minimising diameter. The '- 1' is to compensate for 1-based indexing.
az_min_diameter_sph_mat = idx_min_diameter_az_sph - 180; % The actual azimuthal angle minimising diameter.

% Finding the antipode (in the posterior hemisphere) of the point minimising diameter (in the anterior hemisphere).
[el_min_diameter_antipode_sph_mat, az_min_diameter_antipode_sph_mat] = antipodal_calculator(el_min_diameter_sph_mat, az_min_diameter_sph_mat);
...
if flag_plot == 2
    ...
    hold on; plot3(idx_az_max_diameter_sph, idx_el_max_diameter_sph, max_diameter,'r.','MarkerSize',30);
...
idx_el_max_diameter_antipode_sph = el_max_diameter_antipode_sph_mat + 91;
idx_az_max_diameter_antipode_sph = az_max_diameter_antipode_sph_mat + 180;

idx_el_min_diameter_antipode_sph = el_min_diameter_antipode_sph_mat + 91;
idx_az_min_diameter_antipode_sph = az_min_diameter_antipode_sph_mat + 180;
...
if flag_plot > 0
    ...
    hold on; plot3([radius_x_scg(idx_el_max_diameter_sph, idx_az_max_diameter_sph); radius_x_scg(idx_el_max_diameter_antipode_sph, idx_az_max_diameter_antipode_sph)],[radius_z_scg(idx_el_max_diameter_sph, idx_az_max_diameter_sph); radius_z_scg(idx_el_max_diameter_antipode_sph, idx_az_max_diameter_antipode_sph)],[radius_y_scg(idx_el_max_diameter_sph, idx_az_max_diameter_sph); radius_y_scg(idx_el_max_diameter_antipode_sph, idx_az_max_diameter_antipode_sph)],'r--','LineWidth',3);
    hold on; plot3([radius_x_scg(idx_min_diameter_el_sph, idx_min_diameter_az_sph); radius_x_scg(idx_el_min_diameter_antipode_sph, idx_az_min_diameter_antipode_sph)],[radius_z_scg(idx_min_diameter_el_sph, idx_min_diameter_az_sph); radius_z_scg(idx_el_min_diameter_antipode_sph, idx_az_min_diameter_antipode_sph)],[radius_y_scg(idx_min_diameter_el_sph, idx_min_diameter_az_sph); radius_y_scg(idx_el_min_diameter_antipode_sph, idx_az_min_diameter_antipode_sph)],'b--','LineWidth',3); legend('S1 Acceleration Trajectory','S1 3D Hull Surface','Max 3D Hull Surface Diameter','Min 3D Hull Surface Diameter');
    
    clear max_lim min_lim el_max_diameter_antipode_sph_mat az_max_diameter_antipode_sph_mat el_min_diameter_antipode_sph_mat az_min_diameter_antipode_sph_mat
    
end


%% 4 Troubleshooting - Calculating max and min distances (should equal max S1 p2p and min S1 p2p, respectively).
max_distance = sqrt( (radius_x_scg(idx_el_max_diameter_sph, idx_az_max_diameter_sph) - radius_x_scg(idx_el_max_diameter_antipode_sph, idx_az_max_diameter_antipode_sph)).^2 + (radius_z_scg(idx_el_max_diameter_sph, idx_az_max_diameter_sph) - radius_z_scg(idx_el_max_diameter_antipode_sph, idx_az_max_diameter_antipode_sph)).^2 + (radius_y_scg(idx_el_max_diameter_sph, idx_az_max_diameter_sph) - radius_y_scg(idx_el_max_diameter_antipode_sph, idx_az_max_diameter_antipode_sph)).^2 );
min_distance = sqrt( (radius_x_scg(idx_min_diameter_el_sph, idx_min_diameter_az_sph) - radius_x_scg(idx_el_min_diameter_antipode_sph, idx_az_min_diameter_antipode_sph)).^2 + (radius_z_scg(idx_min_diameter_el_sph, idx_min_diameter_az_sph) - radius_z_scg(idx_el_min_diameter_antipode_sph, idx_az_min_diameter_antipode_sph)).^2 + (radius_y_scg(idx_min_diameter_el_sph, idx_min_diameter_az_sph) - radius_y_scg(idx_el_min_diameter_antipode_sph, idx_az_min_diameter_antipode_sph)).^2 );

clear idx_el_max_diameter_sph idx_az_max_diameter_sph idx_el_max_diameter_antipode_sph idx_az_max_diameter_antipode_sph idx_min_diameter_el_sph idx_min_diameter_az_sph idx_el_min_diameter_antipode_sph idx_az_min_diameter_antipode_sph radius_x_scg radius_y_scg radius_z_scg


%% Converting the function output angles into the SCG reference frame.
el_max_diameter_sph_scg = el_max_diameter_sph_mat;
az_max_diameter_sph_scg = -1*az_max_diameter_sph_mat;
el_min_diameter_sph_scg = el_min_diameter_sph_mat;
az_min_diameter_sph_scg = -1*az_min_diameter_sph_mat;

clear el_max_diameter_sph_mat az_max_diameter_sph_mat el_min_diameter_sph_mat az_min_diameter_sph_mat 

axes_angles = [el_max_diameter_sph_scg, az_max_diameter_sph_scg, el_min_diameter_sph_scg, az_min_diameter_sph_scg]; % The output of the function is the max S1 3D Hull Surface diameter elevation and azimuth angles and the min S1 3D Hull Surface diameter elevation and azimuth angles in the SCG reference frame.
end
