function [avg_max_over_min_diameters] = anisotropy_calculator_avg_max_over_min_diameters(fs,r_pos,scg_xyz)
% This algorithm calculates a simple index of average S1 anisotropy - the ratio of S1 3D trajetctoy max diameter over min diameter. 
...
% Part 2. Identification the maximum and minimum antipodal distances (i.e. diameters) of the surface.

% INPUTS:
% fs - sampling frequency.
% r_pos - R wave peak sample positions.
% scg_xyz - scg signals - the three columns correspond to the 3 x, y, and z components.

% OUTPUT:
% avg_max_over_min_diameters = indicator of anisotropy.
...
max_over_min_diameters = zeros(1,length(r_pos));
...
for i = 1:length(r_pos)
    ...
    %% Part 2.3 Finding maximum and minimum diameters.
    
    % Finding maximum and minimum diameters (i.e. maximum and minimum radius antipodal sum).
    max_diameter = max(max(diameter_sph_mat));
    min_diameter = min(min(diameter_sph_mat));
    
    max_over_min_diameters(i) = max_diameter/min_diameter;
       
end

avg_max_over_min_diameters = mean(max_over_min_diameters);
